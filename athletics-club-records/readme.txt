...
= 0.3.8 =
* Reverted the inclusive age-band semantics introduced in v0.3.7. Brentwood Beagles records use strict club semantics: each performance counts for EXACTLY ONE age-group bucket — the bucket its mapped age_group_at_time falls into. A 14-year-old's PB no longer also holds the U16/U18/U20 cells; if no U16/U18/U20 athlete has a qualifying performance, those cells stay blank.
* Retained the v0.3.7 stale-cell cleanup: recompute still clears all non-override `source='recompute'` record cells before rebuilding, so cells whose qualifying performance moved buckets (or was deleted) no longer leave stale entries.
